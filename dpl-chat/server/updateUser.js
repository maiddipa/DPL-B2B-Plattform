...
async function updateUsers() {
  const dvitServerClientUsers = await dvitServer.queryUsers({
    id: { $nin: ["asd_da_a_a_sdfasdfasd"] },
  });
  console.log("Users", dvitServerClientUsers);

  // for (i = 0; i < dvitServerClientUsers.users.length; i++) {
  //        let user = dvitServer.updateUsers([{
  //         id: dvitServerClientUsers.users[i].id,
  //         role: dvitServerClientUsers.users[i].role,
  //         name: dvitServerClientUsers.users[i].name,
  //         userLanguage: 'de',
  //         email: dvitServerClientUsers.users[i].email
  //       }]);
  //   console.log(dvitServerClientUsers.users[i], "u");
  // }
}
