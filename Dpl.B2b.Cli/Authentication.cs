...
namespace Dpl.B2b.Cli
{
    ...
    {
        public static async Task<AuthenticationResult> AuthenticateDelegated(IEnumerable<string> scopes)
        {
            var appConfig = AppConfig.ReadFromJsonFile("appsettings.json");

            var app = PublicClientApplicationBuilder
                .CreateWithApplicationOptions(appConfig.PublicClientApplicationOptions)
            ...
            try
            {
                result = await app.AcquireTokenInteractive(scopes)
                    ...
                    {
                        //BrowserRedirectSuccess = new Uri("http://localhost")
                    })
                    .ExecuteAsync();

                // the below code is for device code handling
                //result = await app
                //    .AcquireTokenWithDeviceCode(scopes, async (result) =>
                //    {
                //        Process.Start("cmd", $"/C start {result.VerificationUrl}");
                //        //Process.Start(result.VerificationUrl);
                //    })
                //    .ExecuteAsync();
        ...
        public static async Task<AuthenticationResult> AuthenticateApp(IConfiguration configuration, IEnumerable<string> scopes)
        {
            
            var authConfig = configuration.GetSection("AzureAd").Get<AuthenticationConfig>();

            var secureApp = ConfidentialClientApplicationBuilder.Create(authConfig.ClientId)
            ...
            try
            {
                result = await secureApp.AcquireTokenForClient(scopes)
                    .ExecuteAsync();
                Console.ForegroundColor = ConsoleColor.Green;
                Console.WriteLine("Token acquired");
